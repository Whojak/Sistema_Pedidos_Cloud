...
//IMPORTANTE
// Filtrar los campos con el estado "Espera"
$pedidosEspera = [];
if (!empty($values)) {
...
<h1 class="mb-4 text-3xl font-extrabold text-gray-900 dark:text-white md:text-5xl lg:text-6xl"><span class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">Pedidos</span> Solicitados </h1>
</div>
...
<div class="flex justify-center items-center mt-10 px-10"> <!-- Agregado rellenado horizontal -->
  <div class="relative overflow-x-auto shadow-md sm:rounded-lg bg-white dark:bg-gray-800 p-4"> <!-- Agregado rellenado vertical y fondo blanco -->
    <table id="pedidoTable" class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 mb-8">
      ...
      <tbody>
        <?php
        if (!empty($pedidosEspera)) {
          foreach ($pedidosEspera as $row) {
            echo "<tr class='bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600'>";
            echo "<form action='Asignar.php' method='post'>";
            foreach ($row as $cell) {
              echo "<td class='px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white'><input type='hidden' name='pedido[]' value='".htmlspecialchars($cell)."'>$cell</td>";
            }
            echo "<td class='px-6 py-4 whitespace-nowrap text-right text-sm font-medium'>
                    <button type='submit' class='text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2'>Asignar Repartidor</button>
                  </td>"; 
            echo "</form>";
            echo "</tr>";
          }
        } else {
          echo "<tr><td colspan='11' class='px-6 py-4 font-medium text-gray-900 dark:text-white'>No hay pedidos en espera</td></tr>";
        }
        ?>
      </tbody>
    </table>
